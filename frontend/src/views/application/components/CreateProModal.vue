<template>
  <div>
    ...
    <template #title>
      <div style="font-size: 20px">创建项目并添加</div>
    </template> -->
...
import { AppCreateProject } from '@/api/frontend'
...
const submit = () => {
  ...
  AppCreateProject(params)
    .then((res) => {
      // console.log('AppCreateProject', res)
      if (res.code !== 200) {
        message.error(res.message)
        return
      }
      message.success('创建并添加项目成功')
      data.open = false
      emit('success')
      setTimeout(() => {
        clear()
