...
const show = (name, version, language, editable = true) => {
  app.name = name
  app.version = version
...
const getLicenseList = (name = app.name, version = app.version, language = app.language, page = 1, size = 6) => {
  data.spinning = true
  GetComLicenseList({ name, version, language, page, size })
    .then((res) => {
      // console.log('GetComLicenseList', res)
      data.spinning = false
      if (res.code !== 200) {
